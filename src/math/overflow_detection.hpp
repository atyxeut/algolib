...
{
  static_assert(::aal::conjunction<::aal::is_integral<T1>, ::aal::is_integral<T2>>::value, "given operands must be integers");
  assert(lhs >= 0 && rhs >= 0 && "given operands must be nonnegative");
  return rhs > ::aal::lim<TResult>::max() || lhs > ::aal::lim<TResult>::max() - rhs;
}
...
AAL_CONSTEXPR14 bool imul_overflows(T1 lhs, T2 rhs) noexcept
{
  static_assert(::aal::conjunction<::aal::is_integral<T1>, ::aal::is_integral<T2>>::value, "given operands must be integers");
  assert(lhs >= 0 && rhs >= 0 && "given operands must be nonnegative");
  return rhs == 0 ? false : lhs > ::aal::lim<TResult>::max() / rhs;
}
