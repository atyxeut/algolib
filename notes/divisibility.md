...
**_(Proof)_** Write $b_i = y_i a$ for some $y_i \in \mathbb{Z}$, then we have $\sum_{i = 1}^n x_i b_i = a \sum_{i = 1}^n x_i y_i$, thus $a \mid \sum_{i = 1}^n x_i b_i$.

## Division property 4

$\forall a, b, c, d, n \in \mathbb{Z}, n \mid a - b \land n \mid c - d \Rightarrow n \mid a c - b d$.

**_(Proof)_** Write $a - b = k_1 n$ and $c - d = k_2 n$ for some $k_1, k_2 \in \mathbb{Z}$, then we have $a c - b d = (k_1 n + b)(k_2 n + d) - b d = k_1 k_2 n^2 + k_1 d n + k_2 b n = (k_1 k_2 n + k_1 d + k_2 b)n$, thus $n \mid a c - b d$.
...
**_(Proof)_** Write $x = a^d, y = b^d$ and $n = k d$ for some positive integer $k$, then we have $a^n = x^k, b^n = y^k$, then use [this identity](identities.md#the-difference-of-two-nth-powers).

## Euclid's lemma

$\forall a, b, c \in \mathbb{Z}, a \mid b c \land \gcd(a, b) = 1 \Rightarrow a \mid c$.

**_(Proof)_** See [this](https://en.wikipedia.org/wiki/Euclid's_lemma#Proofs).
...
**_(Proof)_** Write $a - b = k_1 n, b - c = k_2 n$ for some $k_1, k_2 \in \mathbb{Z}$, then $a - c = k_1 n + b - (b - k_2 n) = k_1 n + k_2 n \Rightarrow n \mid a - c$.
...
**_(Proof)_** Write $a - b = k_1 n, c - d = k_2 n$ for some $k_1, k_2 \in \mathbb{Z}$, then $a \pm c - (b \pm d) = (k_1 n + b \pm k_2 n \pm d) - (b \pm d) = k_1 n \pm k_2 n + (b \pm d) - (b \pm d) \Rightarrow n \mid a \pm c - (b \pm d)$.
...
**_(Proof)_** Use [Division property 4](#division-property-4).
...
**_(Proof)_** Use [Division property 5](#division-property-5).
...
**_(Proof)_** Let $d = \gcd(x, n)$, $d > 0$ because $n$ is positive, write $x = d x', n = d n'$, and $\gcd(x', n') = 1$ ([gcd property 2](#gcd-property-2)), then $n \mid x a - x b \Rightarrow d n' \mid d x'(a - b) \Rightarrow n' \mid x'(a - b)$ ([Division property 1](#division-property-1)). As a result, $n' \mid a - b$ ([Euclid's lemma](#euclids-lemma)).
...
**_(Proof)_** Let $e = \gcd(a_1 + k, a_1 - a_2, a_1 - a_3, ..., a_1 - a_n) = \gcd(a_1 + k, |a_1 - a_2|, |a_1 - a_3|, ..., |a_1 - a_n|)$, let $d = \gcd(a_1 + k, a_2 + k, ..., a_n + k)$, then $d \mid (a_1 + k) - (a_i + k) = a_1 - a_i$ ([Division property 3](#division-property-3)), thus $d \mid e$. Similarly we can get $e \mid (a_1 + k) - (a_1 - a_i) = a_i + k$ and $e \mid d$, so $d = e$ ([Order relation 2](#order-relation-2)).
...
**_(Proof)_** If $a_1 = a_2 = ... = a_n = 0$, then $x_1 = x_2 = ... = x_n = 0$ satisfies the proposition. If $a_1, a_2, ..., a_n$ are not all $0$, then $a_1^2 + a_2^2 + ... + a_n^2 > 0$, so we can always find a nonnegative $x_1 a_1 + x_2 a_2 + ... + x_n a_n$. Let $d$ be the smallest none, let $e = \gcd(a_1, a_2, ..., a_n)$, then $e \mid d \Rightarrow e \le d$ ([Order relation 2](#order-relation-2)). Next, we prove that $d$ is a common divisor of $a_1, a_2, ..., a_n$. Pick an arbitrary $x_i$, suppose that $d \nmid x_i$, then we can write $x_i = k d + r$ for some integer $k, r \, (0 \le r < d)$. We quickly notice that $r = k d - x_i$ is also a linear combination of $a_1, a_2, ..., a_n$. This contradicts the hypothesis that $d$ is the smallest nonnegative one, so $d \mid x_i$, then $d \le e \Rightarrow d = e$.
...
**_(Proof)_** Let $e = \gcd(a_1, a_2, ..., a_n)$, then $e$ divides any linear combination of $a_1, a_2, ..., a_n$, so $e \mid d$. On the other hand, we have $d \mid e$ ([gcd property 3](#gcd-property-3)), so $e = d$.
